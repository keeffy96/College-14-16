...
public class Lab2_Question4{
	...
	public static void main (String args [])
	{
		...
		boolean found = false; char userInput = 0;
		...
		while(low<=high&&!found)
		{
			...
			middle = (high+low)/2;
			System.out.println("COMPUTER GUESSS: " + array[middle] + " (answer either h for higher\\l for lower\\c for correct)");

			// get in user input
			userInput = scan.next().charAt(0);
			if (userInput==('h'))// search the higher space
			{
				low = middle+1;
			}
			else if(userInput==('l'))
			...
			{
				System.out.println("Invalid input - > ANSWER: h for higher or l for lower or c for correct only ");
			}
		}
